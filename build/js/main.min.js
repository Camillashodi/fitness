...
window.addEventListener('DOMContentLoaded', function () {
  // Utils
  // ---------------------------------
  var settingsList = document.querySelectorAll('[data-time-settings]');
  var contentList = document.querySelectorAll('[data-list]');
  ...
  settingsList.forEach(function (setting) {
    setting.addEventListener('click', function (e) {
      ...
      if (!setting.classList.contains('is-active')) {
        ...
        var chooseSetting = e.target;
        var chooseSettingValue = chooseSetting.dataset.timeSettings;
        var choosenContent = document.querySelector('[data-list="' + chooseSettingValue + '"]');
  ...
  var btnRight = document.querySelector('.slider__button--next');
  var btnLeft = document.querySelector('.slider__button--previous');
  ...
  var carousel = document.querySelector('.trainers__list');
  var carouselElement = document.getElementsByClassName('trainers__item');
  var nextItem = document.querySelector('.trainers__button--next');
  var prvItem = document.querySelector('.trainers__button--previous');
  nextItem.addEventListener('click', function () {
    if (innerWidth > 1999 && carouselElement.length > 4) {
      var elment = carouselElement[0];
      elment.remove();
      carousel.appendChild(elment);
    }

    if (innerWidth > 767 && innerWidth <= 1999 && carouselElement.length > 2) {
      var _elment = carouselElement[0];

      _elment.remove();

      carousel.appendChild(_elment);
    }

    if (innerWidth <= 767 && carouselElement.length > 1) {
      var _elment2 = carouselElement[0];

      _elment2.remove();

      carousel.appendChild(_elment2);
    }
  });
  prvItem.addEventListener('click', function () {
    if (innerWidth > 1999 && carouselElement.length > 4) {
      var elment = carouselElement[carouselElement.length - 1];
      elment.remove();
      carousel.insertAdjacentElement('afterbegin', elment);
    }

    if (innerWidth > 767 && innerWidth <= 1999 && carouselElement.length > 2) {
      var _elment3 = carouselElement[carouselElement.length - 1];

      _elment3.remove();

      carousel.insertAdjacentElement('afterbegin', _elment3);
    }

    if (innerWidth <= 767 && carouselElement.length > 1) {
      var _elment4 = carouselElement[carouselElement.length - 1];

      _elment4.remove();

      carousel.insertAdjacentElement('afterbegin', _elment4);
    }
  }); // --------- form check -----------
  ...
  var videoContainer = document.querySelector('.description__youtube-frame');
  var videoButton = document.querySelector('.description__video-button');
